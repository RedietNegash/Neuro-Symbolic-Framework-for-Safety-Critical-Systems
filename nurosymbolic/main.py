...
def setup_environment():
    ...
    load_dotenv(override=True)  # Ensure .env overrides environment variables
...
def main():
    """Main demonstration of the neuro-symbolic verification framework with Gemini or Llama"""
    print("Neuro-Symbolic Code Verification Framework")
    print(f"Using {'Gemini' if config.ACTIVE_LLM.lower() == 'gemini' else 'Llama'} as LLM backend")
    print("=" * 60)
    ...
    try:
        ...
        llm_client = (GeminiLLMClient() if config.ACTIVE_LLM.lower() == "gemini" 
                     else LlamaLLMClient())
